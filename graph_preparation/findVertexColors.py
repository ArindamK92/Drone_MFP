...
##AK::scaling down as we are using graph of very large area, but in real it will be smaller than this
def compute_edge_weight(u, v, payload_weight, drone_speed, global_wind_speed, global_wind_direction):
    ...
    weight = prefixes[(payload_weight, drone_speed, global_wind_speed, relative_wind_direction)] * distance/15 #scaling down wt by factor 15 just for computational simplicity
...
def update_graph(G, params):
    ...
    for u, vdict in G.adjacency():
        for v in vdict:
            G[u][v]['weight'] = compute_edge_weight(u, v, payload_weight, drone_speed, global_wind_speed,
...
#                             global_wind_speed = np.random.choice(winds)
#                             global_wind_direction = np.random.randint(360)
#                             edge_w = compute_edge_weight(path[0][j], path[0][j + 1], max_payload, drone_speed,
#                                                          global_wind_speed, global_wind_direction)
#                             actual_len_go = actual_len_go + edge_w
#                         for j in range(0, len(path[1]) - 1):
#                             global_wind_speed = np.random.choice(winds)
#                             global_wind_direction = np.random.randint(360)
#                             edge_w = compute_edge_weight(path[1][j], path[1][j + 1], 0, drone_speed, global_wind_speed,
#                                                          global_wind_direction)
...
#
#                 print("Gray=%d -> abort=%d, success=%d, delivered=%d, fail=%d" % (
#                 n_gray, s_abort, s_success, s_delivered, s_fail))
#
...
#
#                             # print("%d: gray (exp=%d, act=%d)" % (i, expected_len, actual_len))
#                 print("Gray=%d -> success=%d, delivered=%d, fail=%d, fail_noway=%d" % (
#                 n_gray, s_success, s_delivered, s_fail, s_fail_noway))
#
#                 exp_statuses[i][1] = 100. * s_success / n_gray
#                 exp_statuses[i][2] = 100. * s_delivered / n_gray
#                 exp_statuses[i][3] = 100. * s_fail / n_gray
...
if __name__ == '__main__':
    ...
    for u, vdict in G.adjacency():
        for v in vdict:
            ...
            directions[u][v] = get_bearing(G._node[u]['Latitude'], G._node[v]['Latitude'], G._node[u]['Longitude'],
                                           G._node[v]['Longitude'])
    ...
    color = alg_pre_proc(G, params)
    #white: 0 grey:1 black:2
    file1 = open("colors.txt", "w")
    for i in range(n):
        str1 = "{} {}\n".format(i+1, color[i]) #modifying vertex starting from 1
        print(str1)
        file1.write(str1)
    file1.close()
